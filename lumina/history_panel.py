"""History panel for clipboard archive browsing."""
import ctypes
import io
import json
import os
...
class HistoryPanel:
    ...
    def _open_file_from_row(self, row):
        def _action():
            ...
            path = self._resolve_file_path(full)
            if path and os.path.exists(path):
                try:
                    os.startfile(path)
                except OSError:
                    self.status_var.set("文件无法打开")
            else:
                self.status_var.set("文件不存在或归档不可用")
        return _action
    ...
    def _resolve_file_path(self, row):
        """Prefer the source file, then the archive, then materialize a BLOB."""
        if not row:
            return ""
        source = (row["content"] or "").splitlines()[0].strip()
        if source and os.path.isfile(source):
            return source

        file_data = self.db.get_file_data(row["id"])
        if not file_data:
            return ""
        stored = file_data.get("file_path") or ""
        ...
        if not data:
            return ""
        folder = os.path.join(tempfile.gettempdir(), "Lumina", "files")
        os.makedirs(folder, exist_ok=True)
        name = os.path.basename(source) or f"file_{row['id']}.bin"
        path = os.path.join(folder, f"{row['id']}_{name}")
        try:
...
class DetailPane:
    ...
    def _build_image_body(self, outer, row, tk):
        ...
        self._body.bind("<ButtonRelease-1>", self._open_preview_externally)
        self._body.bind("<MouseWheel>", self._on_wheel)

    def _open_preview_externally(self, _event=None):
        """Open the preview with the system viewer, matching the file Open action."""
        if self.row is None:
            return "break"
        if self.cat == "file":
            self._open_file()
        ...
        data = self.row["data"]
        if not data:
            self.panel.status_var.set("图片数据不可用")
            return "break"
        folder = os.path.join(tempfile.gettempdir(), "Lumina", "images")
        try:
            os.makedirs(folder, exist_ok=True)
            path = os.path.join(folder, f"clip_{self.row['id']}.png")
            with open(path, "wb") as stream:
                stream.write(data)
            os.startfile(path)
        except OSError:
        ...
        if (status == "ready" and path
            ...
            if image_data:
                try:
                    ...
                    image_label.bind("<ButtonRelease-1>", self._open_preview_externally)
                except Exception:
                    pass
    ...
    def _file_row(self, parent, tk, path, T):
        resolved_path = self.panel._resolve_file_path(self.row)
        display_path = path
        open_path = resolved_path or path
        exists = os.path.exists(open_path)
        is_dir = os.path.isdir(open_path)
        ...
        for w in (fr, nl, info_label):
            if w is None:
                continue
            w.bind("<Button-1>", lambda e, p=open_path: self._open_path(p))
            w.bind("<Double-Button-1>", lambda e, p=open_path: self._open_path(p))
            w.bind("<Button-3>", lambda e, p=open_path: self._file_menu(p, e))
            w.bind("<MouseWheel>", self._on_wheel)
    ...
    def _file_menu(self, path, e):
        path = self.panel._resolve_file_path(self.row) or path
        items = []
        if os.path.exists(path):
    ...
    def _open_file(self):
        path = self.panel._resolve_file_path(self.row)
        if not path:
            return
        try:
            os.startfile(path)
