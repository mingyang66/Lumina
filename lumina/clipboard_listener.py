...
class ClipboardListener(threading.Thread):
    ...
    def write_clipboard(self, writer, payload):
        ...
        with self._ignore_lock:
            before = clipboard_api.get_clipboard_sequence()
            ok = writer(payload)
            after = clipboard_api.get_clipboard_sequence()
    ...
    def _handle_clipboard(self):
        if self._consume_ignore(clipboard_api.get_clipboard_sequence()):
            return
        try:
            # 文件复制（CF_HDROP）优先：资源管理器复制文件时剪贴板可能同时带文本
            files = clipboard_api.get_clipboard_files()
            if files:
                self._enqueue(("files", files,
                               clipboard_api.get_foreground_process()))
                return
            text = clipboard_api.get_clipboard_text()
            dib = None if text is not None else clipboard_api.get_clipboard_dib()
            if text is None and dib is None and clipboard_api.clipboard_has_content():
                # 多程序争用剪贴板时可能瞬时读取失败，稍候重试一次
                time.sleep(0.08)
                text = clipboard_api.get_clipboard_text()
                if text is None:
                    dib = clipboard_api.get_clipboard_dib()
                    if dib is None:
                        files = clipboard_api.get_clipboard_files()
                        if files:
                            self._enqueue(("files", files,
                                           clipboard_api.get_foreground_process()))
                            return
            source = clipboard_api.get_foreground_process()
            if text is not None:
